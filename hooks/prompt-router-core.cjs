...
const ROUTE_ACTION = /\b(implement|build|create|add|change|modify|fix|debug|refactor|review|test|ship|deploy|release|migrate|integrate|implementar|implementa|construir|crear|agregar|agrega|cambiar|modificar|arreglar|arregla|corregir|corrige|depurar|depura|refactorizar|revisar|revisa|probar|prueba|desplegar|publicar|migrar|integrar)\b/i;
const WORK_CONTEXT = /\b(repo(?:sitory)?|code(?:base)?|code|diff|project|feature|bug|regression|tests?|pull request|pr|release|deploy|mcp|api|workflow|specs?|retro|repositorio|c[oó]digo|proyecto|funcionalidad|error|regresi[oó]n|pruebas?|cambio|implementaci[oó]n|lanzamiento|despliegue|retrospectiva)\b/i;
